...
export default function ScienceDiagramView({ diagram, compact = false }: { diagram?: ScienceDiagram | null; compact?: boolean }) {
  ...
  if (!diagram || !Array.isArray(diagram.primitives) || (diagram.primitives.length < 2 && !diagram.table && !diagram.libraryImages?.length)) return null;
  ...
  return (
    <figure className={`science-diagram-card${compact ? " science-diagram-compact" : ""}`}>
      ...
      {diagram.primitives.length >= 2 && <svg className="science-diagram-svg" viewBox="0 0 100 100" role="img" aria-label={diagram.title || "科學圖解"}>
      ...
      {diagram.table && <div className="science-table-scroll"><table><thead><tr>{diagram.table.headers.map((header,index)=><th key={index} scope="col">{header}</th>)}</tr></thead><tbody>{diagram.table.rows.map((row,index)=><tr key={index}>{row.map((cell,column)=><td key={column}>{cell}</td>)}</tr>)}</tbody></table></div>}
      {diagram.libraryImages?.map(asset=><div key={asset.id} className="science-library-figure"><img src={`/api/teaching-images/${encodeURIComponent(asset.id)}`} alt={asset.title}/><strong>{asset.title}</strong><p>教師講義參考圖</p></div>)}
      ...
      <style jsx global>{`
        .history-export-paper{--text:#243447;--text-secondary:#526476;--text-muted:#526476;--primary:#276c78;--surface:#fff;--surface-soft:#f1f5f7;--border:#ccd6df}
        .science-library-figure{margin:14px 0}.science-library-figure img{display:block;width:100%;height:auto;max-height:650px;object-fit:contain;background:white;border-radius:10px}.science-library-figure strong{display:block;font-size:13px;margin-top:8px}.science-library-figure p{font-size:11px;color:var(--text-secondary)}.science-table-scroll{overflow:auto;margin:12px 0}.science-table-scroll table{width:100%;border-collapse:collapse;font-size:13px}.science-table-scroll th,.science-table-scroll td{border:1px solid var(--border);padding:10px;text-align:left;min-width:65px;white-space:pre-wrap}.science-table-scroll th{background:var(--surface-soft)}
