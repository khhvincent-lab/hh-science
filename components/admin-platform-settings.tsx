...
export default function AdminPlatformSettings({ onBrandChanged }: { onBrandChanged?:()=>void }) {
  ...
  async function addTeacher(){
    ...
    try{const r=await fetch("/api/admin/teachers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username,displayName,password,classIds})});const d=await r.json();if(!r.ok)throw new Error(d.error||"建立教師失敗。");setUsername("");setDisplayName("");setPassword("");setClassIds([]);setMessage("教師帳號已建立。");await load();}catch(e){setError(e instanceof Error?e.message:"建立教師失敗。");}finally{setBusy(false);}
  }
  ...
  return <div className="admin-platform-stack">
    ...
    <section className="hh-card admin-panel">
      ...
      <div className="admin-settings-grid">
        ...
        <label className="admin-field"><span>老師姓名</span><input className="hh-input" value={displayName} onChange={e=>setDisplayName(e.target.value)} placeholder="例如 王老師"/></label>
        <label className="admin-field"><span>初始密碼</span><input className="hh-input" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="至少 8 碼"/></label>
      </div>
      <div className="teacher-class-picker"><strong>可管理班級</strong><div className="teacher-class-grid">{classes.map(c=><label key={c.id} className="teacher-class-chip"><input type="checkbox" checked={classIds.includes(c.id)} onChange={e=>setClassIds(e.target.checked?[...classIds,c.id]:classIds.filter(x=>x!==c.id))}/><span>{c.name}</span></label>)}</div></div>
      <div className="admin-actions"><button className="hh-button-primary" type="button" onClick={()=>void addTeacher()} disabled={busy}>新增教師帳號</button></div>
