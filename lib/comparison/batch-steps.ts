...
import {historyRecoveryCounts} from './recovery';
...
export async function nextHistoryChunk(id:string){
 ...
 let reason='';
 const recovery=await historyRecoveryCounts(id);
 if(progress.running)reason='先前模型呼叫結果尚未確認，已暫停，避免重複計費。';
 if(recovery.blocked)reason='修復後的試跑失敗或累積三次失敗，已暫停；請檢查回覆格式及模型錯誤紀錄。';
 ...
 const {data:items,error:ie}=await db.from('model_comparison_cases').select('id').eq('batch_id',id).eq('b_status','queued').order('created_at').order('id').limit(recovery.succeeded?4:1);if(ie)throw ie;
