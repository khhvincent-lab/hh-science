...
3. 圖片有效後，再判斷是否屬於自然科。
4. 實質跨自然科的題目（例如氣體定律、理想氣體、分壓、氣體動力論、熱力學、電解池搭配電路計算，或生物化學跨科）請用 category=mixed_science，allowed=true。這些主題可以同時在物理與化學等科目教授，不能因學生選了其中一科就阻擋；也不能只因題目出現電流、化學式等單一詞彙就推定為另一科。
...
挑選真正有教學價值的數字、關鍵觀念、公式片段、單位換算與推理轉折，不限於單純數字。
在資訊足夠時，目標標註 8～12 個真正有學習價值的互動重點（題目內容不足則少於 8 個，不為湊數編造）：可以是常數、關鍵中間值、變數、單位、公式片段、化學式、換算因子或臨界值。不要標題號／步驟號，也不要為了湊數硬標。
...
優先考慮：
- 物理：受力、斜面、圓周運動、彈簧、滑輪、光學、簡單電路、向量、波形、v-t／x-t／a-t 圖、簡單座標圖
- 地科：地層／斷層、板塊、日地月、太陽入射角、大氣與海洋環流、剖面圖與簡單變化趨勢圖
- 化學：簡單實驗裝置、粒子／流程幾何關係、相圖或簡單變化圖；結構式請交給 chemicalStructure
...
8. 畫座標圖時，座標系仍使用 0～100；畫面 y 座標向下增加，所以數值越大通常應放在越靠上的位置。座標軸可用 arrow，曲線可用 line / polyline，刻度與單位用 label。
9. confidence 只有你對圖的科學關係有把握時才給 80 以上；若低於 70，直接 diagram=null。
10. 教師規則若明確要求某類題「應附圖」或指定圖中重點，優先遵循，但仍須符合題目事實。
