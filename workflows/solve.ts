import {queueCompletedComparison} from '@/lib/comparison/dispatch';
...
export async function solveWorkflow(id:string){
 ...
 try{
  ...
  const result=await completeJob(id,routed,quota);
  // Student result is already committed. Comparison failure must never fail/refund it.
  try { await queueCompletedComparison(result.historyId); } catch {}
  return result;
 }catch{
