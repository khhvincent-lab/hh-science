...
本版為公開閱讀區，作答當次即時計分且不扣AI額度；後台「班務管理 → 專欄成效」提供閱讀與三題交卷統計，但尚無學生端跨裝置答題紀錄、收藏或後台文章編輯器。
...
- 開啟文章後由瀏覽器送出 view；預先載入連結不計次。每次開啟使用獨立 UUID，同次重送不重複計次。
- 完成所有題目並提交才送出 complete，由伺服器驗證選項、判分、識別登入學生；保留本次閱讀的首次交卷。統計失敗不阻擋閱讀或本機詳解，可重試同步。
- 公開文章的未登入瀏覽僅計次，不推估人數，不列入學生完成率。老師只能看到所管理班級目前學生的資料；只有未切換教師範圍的超級管理員能看到未登入瀏覽。
- 最近 7／30 天是連續小時區間，以文章開啟時間選取閱讀紀錄，交卷統計截止於報表更新時間。完成率＝該期間閱讀者中至少完成一篇三題者／該期間閱讀者。每篇同理；不要求答對。
- 總人數按登入學生 ID 去重，每篇內亦去重。各篇人數不宜相加。未登入後再登入，不追溯合併先前的匿名瀏覽。開啟不等於讀完；不回填功能上線前的數字。
- 資料表 chemistry_reading_visits 啟用 RLS 並撤銷 anon/authenticated 權限，僅伺服器 service_role 存取。不記 IP、指紋或學生姓名。API 報表禁止快取。
