...
async function adminTeachingFilesToDataUrls(files: FileList | null) {
  ...
  return Promise.all(picked.map((file) => new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error("讀取詳解圖片失敗。"));
    reader.onload = () => {
...
function StudentsSection(props: {
  ...
  const [createStudentNotice, setCreateStudentNotice] = useState<{ text: string; error: boolean } | null>(null);
  ...
  async function createStudent() {
    ...
    if (!name || !regionId || !institutionId || !classId) {
      setCreateStudentNotice({
        text: !name ? "請輸入學生姓名。" : "請完整選擇地區、合作單位與班級。",
        error: true,
      });
      return;
    }
    ...
    setOrgBusy(true);
    setCreateStudentNotice({ text: "正在新增學生…", error: false });
    try {
      const response = await fetch("/api/admin/students", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, regionId, institutionId, classId }),
      });
      const data = await response.json().catch(() => null);
      if (!response.ok) throw new Error(data?.error || `新增學生失敗（HTTP ${response.status}）。`);

      props.setNewName("");
      setCreateStudentNotice({ text: `已新增 ${name}。`, error: false });
      try {
        await props.reloadStudents();
      } catch {
        setCreateStudentNotice({ text: `已新增 ${name}，但名單更新失敗。請重新整理確認，勿重複新增。`, error: true });
      }
    } catch (error) {
      setCreateStudentNotice({
        text: error instanceof Error ? error.message : "新增學生失敗。",
        error: true,
      });
    } finally {
      setOrgBusy(false);
    ...
    <div className={`admin-stack org-students-v12 mode-${props.mode}`}>
      ...
      <section className="hh-card admin-panel org-compact-panel student-only student-create-hub">
        ...
        <div className="org-add-row">
          ...
          <button type="button" className="hh-button-primary" onClick={() => void createStudent()} disabled={orgBusy}>{orgBusy ? "新增中…" : "新增"}</button>
        </div>
        {createStudentNotice && (
          <div role="status" aria-live="polite" className={`admin-notice ${createStudentNotice.error ? "danger" : "success"} student-create-notice`}>
            {createStudentNotice.text}
          </div>
        )}
      </div>
      ...
      <style jsx global>{`
        ...
        @media (max-width: 760px) {
          .class-overview-toolbar{margin:6px 0 7px;gap:7px}.class-overview-toolbar .hh-select{max-width:none;flex:1;min-height:38px;height:38px;padding-top:0;padding-bottom:0;font-size:13px}.class-overview-toolbar span{font-size:10px}.class-overview-grid{border:1px solid var(--border);border-radius:10px;overflow:hidden}.class-overview-grid-head,.class-overview-grid-row{display:grid;grid-template-columns:minmax(92px,1.7fr) repeat(5,minmax(0,.72fr));gap:0;align-items:center}.class-overview-grid-head{background:var(--surface-soft);border-bottom:1px solid var(--border);margin:0}.class-overview-grid-head button{min-width:0;font-size:9px;padding:7px 2px;border:0;border-right:1px solid var(--border);border-radius:0;background:transparent;text-align:center;white-space:nowrap}.class-overview-grid-head button:first-child{text-align:left;padding-left:8px}.class-overview-grid-head button:last-child{border-right:0}.class-overview-grid-row{border:0;border-bottom:1px solid var(--border);border-radius:0;margin:0;padding:5px 2px;min-height:42px}.class-overview-grid-row:last-child{border-bottom:0}.class-overview-name{grid-column:auto;border:0;padding:0 5px;min-width:0;gap:0}.class-overview-name strong{font-size:12px;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.class-overview-name small{font-size:8px;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.class-overview-grid-row>span{display:flex;align-items:baseline;justify-content:center;gap:1px;padding:0;min-width:0}.class-overview-grid-row>span b{font-size:11px;line-height:1;white-space:nowrap}.class-overview-grid-row>span small{display:none}.org-compact-panel.class-only{padding:12px}.org-compact-panel.class-only .hh-panel-header{margin-bottom:4px}.org-compact-panel.class-only .hh-panel-header p{display:none}.student-create-body{padding:0 10px 10px}.student-create-subpanel{padding:10px}.student-create-toggle{padding:14px}.student-create-toggle strong{font-size:18px}
...
function AdminNavGroup({
  ...
  return (
    <div className={`admin-nav-group ${open ? "open" : ""} ${active ? "active" : ""}`}>
      <button type="button" className="admin-nav-group-toggle" onClick={onToggle}>
...
function UsageStatusSection({ dashboard }: { dashboard: DashboardData | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/admin/class-overview", { cache:"no-store" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "讀取使用狀況失敗。");
      setRows(Array.isArray(data.rows) ? data.rows : []);
    } catch (e) { setError(e instanceof Error ? e.message : "讀取使用狀況失敗。"); }
...
function TeachingQuestionsSection({initialHistoryId,onInitialHistoryHandled,canEdit=true,canSaveGlobalRules=true}:{initialHistoryId?:string|null;onInitialHistoryHandled?:()=>void;canEdit?:boolean;canSaveGlobalRules?:boolean} = {}) {
  ...
  const [teacherReferenceImages,setTeacherReferenceImages]=useState<string[]>([]);
  const [coachReferenceImages,setCoachReferenceImages]=useState<string[]>([]);
  ...
  async function open(item:TeachingQuestionRow){
    ...
    setCoachInput("");
    setCoachMessages([]);
    setTeacherReferenceImages([]);
    setCoachReferenceImages([]);
    try{
  ...
  async function generateAnnotations(){
    ...
    try{
      ...
      const data=await response.json();
      if(!response.ok)throw new Error(data.error||"互動重點整理失敗。");
      if(data.explanation)setTeacherExplanation(data.explanation);
      if(Array.isArray(data.annotations))setTeacherAnnotations(data.annotations);
      setMessage(`已重新整理互動重點，目前 ${Array.isArray(data.annotations)?data.annotations.length:0} 個。請確認後再儲存。`);
    }catch(e){setMessage(e instanceof Error?e.message:"互動重點整理失敗。");}
    finally{setAnnotationBusy(false);}
  }

  async function sendCoach(){
    if(!selected||coachBusy||(!coachInput.trim()&&!coachReferenceImages.length))return;
    const coachText=coachInput.trim()||(coachReferenceImages.length?"請直接讀取我附上的詳解圖片，整理我的解題順序與教學偏好。":"");
    const next=[...coachMessages,{role:"user" as const,content:coachText}];
    setCoachMessages(next);setCoachInput("");setCoachBusy(true);setMessage("");
    try{
      const response=await fetch("/api/admin/teaching-coach",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({task:"chat",subject:selected.subject,questionNote:selected.questionNote,referenceAnswer:selected.referenceAnswer,aiAnswer:selected.answer,aiExplanation:selected.explanation,teacherAnswer,teacherExplanation,teacherOptions,teacherStrategy,teacherNote,topic,keywords,questionSignature,messages:next,images:coachReferenceImages})});
  ...
  if(selected){return <div className="admin-stack teaching-question-detail teacher-calibration-v2">
    ...
    <section className="hh-card admin-panel teacher-annotation-panel"><div className="teacher-calibration-head"><div><div className="hh-eyebrow">INTERACTIVE NUMBERS</div><h2 className="hh-display">互動式詳解管理</h2><p>不再只靠模型偶爾標幾個。你可以讓 AI 重新挑 4～8 個，或手動新增／修改每個數值、變數、單位或公式片段的意義、來源與用途。</p></div><button type="button" className="hh-button-secondary" onClick={()=>void generateAnnotations()} disabled={annotationBusy||!teacherExplanation.trim()}>{annotationBusy?"整理中…":"AI 建議互動重點"}</button></div>
      <div className="teacher-annotation-summary"><strong>{teacherAnnotations.length}</strong><span>個互動重點</span><small>儲存後會同步到學生這題的詳解；相似題也會把這種標註密度當成教師範例。</small></div>
      <div className="teacher-annotation-list">{teacherAnnotations.map((a,index)=><article key={`${a.id}-${index}`}><div className="teacher-annotation-head"><input className="hh-input" value={a.display} onChange={e=>updateAnnotation(index,{display:e.target.value})} placeholder="顯示內容，例如 22.4、R、K_a"/><input className="hh-input" value={a.label} onChange={e=>updateAnnotation(index,{label:e.target.value})} placeholder="名稱，例如：莫耳質量"/><button type="button" onClick={()=>setTeacherAnnotations(v=>v.filter((_,i)=>i!==index))}>刪除</button></div><textarea className="hh-input" value={a.meaning} onChange={e=>updateAnnotation(index,{meaning:e.target.value})} placeholder="這個互動重點代表什麼"/><div className="teacher-annotation-two"><input className="hh-input" value={a.source} onChange={e=>updateAnnotation(index,{source:e.target.value})} placeholder="如何得到／從哪裡來"/><input className="hh-input" value={a.usage} onChange={e=>updateAnnotation(index,{usage:e.target.value})} placeholder="這一步為什麼用它"/></div></article>)}{teacherAnnotations.length===0&&<div className="admin-empty">目前沒有互動重點。按「AI 建議互動重點」會依老師版詳解重新整理。</div>}</div>
      <button type="button" className="hh-button-secondary teacher-add-annotation" onClick={()=>setTeacherAnnotations(v=>[...v,{id:`t${v.length+1}`,display:"",label:"",meaning:"",source:"",usage:""}])}>＋ 手動新增互動重點</button>
...
@media (max-width: 760px) {
  ...
  .teaching-toolbar p { margin:0; color:var(--text-secondary); font-size:12px; }
  .teaching-filter-row { display:grid; grid-template-columns:auto minmax(0,1fr) 150px auto; gap:9px; }
  .teaching-range-switch{display:flex;padding:3px;border:1px solid var(--border);border-radius:12px;background:var(--surface-soft)}.teaching-range-switch button{min-width:54px;border:0;border-radius:9px;background:transparent;color:var(--text-secondary);font-size:11px;font-weight:800;cursor:pointer}.teaching-range-switch button.active{background:var(--surface);color:var(--primary);box-shadow:var(--shadow-sm)}
  .teaching-question-list { display:grid; gap:8px; }
  .teaching-question-row { width:100%; display:grid; grid-template-columns:92px minmax(0,1fr) 94px; align-items:center; gap:13px; padding:9px; color:var(--text); text-align:left; cursor:pointer; }
  .teaching-question-row>img,.teaching-thumb-empty { width:92px; height:68px; object-fit:cover; border-radius:11px; background:var(--surface-soft); }
  .teaching-thumb-empty { display:grid; place-items:center; color:var(--primary); font-size:11px; font-weight:900; letter-spacing:.1em; }
  .teaching-question-main { display:grid; gap:4px; min-width:0; }
  .teaching-row-meta { color:var(--primary); font-size:10px; font-weight:850; }
  .teaching-question-main strong { overflow:hidden; font-size:13px; text-overflow:ellipsis; white-space:nowrap; }
  .teaching-question-main small { color:var(--text-secondary); font-size:10px; }
  .teaching-row-answer { display:grid; justify-items:end; gap:3px; }
  .teaching-row-answer b { color:var(--primary); font-size:17px; }
  .teaching-row-answer em { padding:3px 6px; border-radius:999px; background:var(--danger-soft); color:var(--danger); font-size:9px; font-style:normal; font-weight:850; }
  .teaching-row-answer span { color:var(--text-secondary); font-size:10px; font-weight:800; }
  .teaching-detail-head { display:flex; align-items:flex-start; justify-content:space-between; gap:14px; }
  .teaching-detail-head h2 { margin:5px 0 4px; }
  .teaching-detail-head p { margin:0; color:var(--text-secondary); font-size:11px; }
  .teaching-issue-badge { padding:6px 9px; border-radius:999px; background:var(--danger-soft); color:var(--danger); font-size:10px; font-weight:850; }
  .teaching-question-image { margin-top:14px; padding:10px; border:1px solid var(--border); border-radius:14px; background:#fff; }
  .teaching-question-image img { display:block; width:100%; max-height:520px; object-fit:contain; }
  .teaching-answer-grid { display:grid; grid-template-columns:1fr 1fr; gap:9px; margin-top:12px; }
  .teaching-answer-grid>div { display:grid; gap:3px; padding:11px 12px; border:1px solid var(--border); border-radius:12px; background:var(--surface-soft); }
  .teaching-answer-grid span { color:var(--text-secondary); font-size:10px; font-weight:800; }
  .teaching-answer-grid strong { color:var(--primary); font-size:18px; }
  .teaching-ai-block { padding:12px 0; border-top:1px solid var(--border); }
  .teaching-ai-block h3 { margin:0 0 8px; font-size:14px; }
  .teaching-model-line { margin-top:12px; color:var(--text-secondary); font-size:10px; }
  .teacher-solution-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
  .teacher-solution-grid label,.teacher-solution-field { display:grid; gap:6px; }
  .teacher-solution-grid label>span,.teacher-solution-field>span { color:var(--text-secondary); font-size:11px; font-weight:800; }
  .teacher-solution-field { margin-top:10px; }
  .teacher-solution-field textarea { min-height:80px; padding-top:10px!important; resize:vertical; }
  .teacher-solution-field .teacher-method-textarea { min-height:150px; }
  .teacher-save-button { width:100%; margin-top:12px; }
  .teaching-mode-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:9px; }
  .teaching-mode-grid button { display:grid; gap:5px; min-height:92px; padding:12px; border:1px solid var(--border); border-radius:14px; background:var(--surface-soft); color:var(--text); text-align:left; cursor:pointer; }
  .teaching-mode-grid button.active { border-color:var(--primary); background:color-mix(in srgb,var(--primary) 9%,var(--surface)); box-shadow:0 0 0 2px color-mix(in srgb,var(--primary) 10%,transparent); }
  .teaching-mode-grid strong { font-size:13px; }
  .teaching-mode-grid span { color:var(--text-secondary); font-size:10px; line-height:1.5; }
  .teaching-rule-checks { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }
  .teaching-rule-checks label { display:flex; align-items:center; gap:8px; min-height:42px; padding:0 10px; border:1px solid var(--border); border-radius:12px; background:var(--surface-soft); font-size:11px; font-weight:800; }
  .teaching-subject-rules { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
  .teaching-subject-rules label { display:grid; gap:6px; }
  .teaching-subject-rules label>span { color:var(--text-secondary); font-size:11px; font-weight:850; }
  .teaching-subject-rules textarea { min-height:105px; padding-top:10px!important; resize:vertical; }
  ...
  /* Teacher knowledge image learning + design-system polish */
  .teacher-reference-upload{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 13px;margin:12px 0 8px;border:1px dashed var(--border);border-radius:14px;background:var(--surface-soft)}
  .teacher-reference-upload>div{display:grid;gap:4px;min-width:0}.teacher-reference-upload strong{font-size:11px}.teacher-reference-upload span{font-size:9px;line-height:1.55;color:var(--text-secondary)}
  .teacher-image-upload-button{position:relative;overflow:hidden;white-space:nowrap}.teacher-image-upload-button input,.teacher-coach-image-bar input{position:absolute;inset:0;opacity:0;cursor:pointer}
  .teacher-reference-thumbs{display:flex;gap:8px;overflow:auto;padding:2px 0 10px}.teacher-reference-thumbs>div{position:relative;flex:0 0 82px;height:82px;border:1px solid var(--border);border-radius:12px;overflow:hidden;background:var(--surface-soft)}.teacher-reference-thumbs img{width:100%;height:100%;object-fit:cover}.teacher-reference-thumbs button{position:absolute;right:4px;top:4px;width:24px;height:24px;border:0;border-radius:999px;background:rgba(0,0,0,.68);color:#fff;font-weight:900}
  .teacher-coach-image-bar{display:flex;align-items:center;gap:8px;padding:9px 10px;border-top:1px solid var(--border);background:var(--surface-soft);font-size:9px;color:var(--text-secondary)}.teacher-coach-image-bar span{margin-right:auto}.teacher-coach-image-bar label{position:relative;overflow:hidden;padding:6px 9px;border:1px solid var(--border);border-radius:999px;color:var(--text);font-weight:800;cursor:pointer}.teacher-coach-image-bar button{border:0;background:transparent;color:var(--danger);font-size:9px}
  .hh-card,.admin-panel{border-radius:16px}.hh-button-primary,.hh-button-secondary,.hh-input,.hh-select{border-radius:12px}.hh-display{letter-spacing:-.02em}
  @media(max-width:760px){.teacher-reference-upload{align-items:stretch;flex-direction:column}.teacher-reference-upload .teacher-image-upload-button{width:100%;justify-content:center}.teacher-reference-thumbs>div{flex-basis:72px;height:72px}.teacher-coach-image-bar{flex-wrap:wrap}.teacher-coach-image-bar span{width:100%}}
