...
function Trend({ rows }: { rows: Insights["daily"] }) {
  const points = (key: "questions" | "costUsd") => {
    const max = Math.max(1, ...rows.map((row) => row[key]));
    return rows.map((row, index) =>
      `${40 + index * 600 / Math.max(1, rows.length - 1)},${181 - row[key] / max * 145}`
    ).join(" ");
  };
  if (!rows.length || rows.every((row) => row.questions === 0 && row.costUsd === 0)) {
    return <div className={styles.emptyChart}>所選期間尚無解題與成本紀錄。</div>;
  }
  const ticks = rows.length <= 7 ? rows : rows.filter((_, i) => i % 5 === 0 || i === rows.length - 1);
  return <div className={styles.chart}>
    <svg viewBox="0 0 680 210" role="img" aria-label="每日解題量與 AI 成本趨勢，兩條曲線分別依各自最大值呈現">
      {[40, 87, 134, 181].map((y) => <line key={y} x1="40" y1={y} x2="640" y2={y} className={styles.gridline} />)}
      <polyline points={points("questions")} className={styles.questionsLine} />
      <polyline points={points("costUsd")} className={styles.costLine} />
      {ticks.map((row) => {
        const index = rows.indexOf(row);
        return <text key={row.day} x={40 + index * 600 / Math.max(1, rows.length - 1)} y="204" textAnchor="middle" className={styles.axisLabel}>{row.day.slice(5)}</text>;
      })}
    </svg>
    <div className={styles.chartNote}>青色為解題題數 · 琥珀色為成本；兩條線各用自己的刻度，數值請見下方每日資料。</div>
    <div className={styles.dailyStrip}>{rows.slice(-7).map((row) => <div key={row.day}><span>{row.day.slice(5)}</span><strong>{row.questions} 題</strong><small>{money(row.costUsd)}</small></div>)}</div>
  </div>;
}
...
export default function DashboardV211({
  ...
  onNavigate: (section: "usage" | "cost" | "teachingQuestions") => void;
}) {
  ...
  const quietClasses = classes.filter((row) => row.students > 0 && row.todayQuestions === 0).length;
  ...
  return <div className={styles.dashboard}>
    ...
    <div className={styles.columns}>
      <div className={styles.primary}>
        <section className={styles.panel}><div className={styles.panelHead}><div><h3>使用與成本趨勢</h3><p>解題量與 AI 成本的每日變化</p></div><div className={styles.segment}><button type="button" className={range === 7 ? styles.active : ""} onClick={() => setRange(7)}>7 天</button><button type="button" className={range === 30 ? styles.active : ""} onClick={() => setRange(30)}>30 天</button></div></div><div className={styles.legend}><span><i className={styles.cyanDot} />解題量</span><span><i className={styles.amberDot} />AI 成本</span></div>{extraLoading && !insights ? <div className={styles.emptyChart}>正在讀取趨勢…</div> : <Trend rows={insights?.daily ?? []} />}</section>
