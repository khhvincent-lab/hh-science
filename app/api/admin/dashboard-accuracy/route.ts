...
type SolveAnswer = { answer: string | null; reference_answer: string | null };
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (studentIds?.length === 0) return NextResponse.json({ referenceCases: 0, referenceMatches: 0 });
    ...
    for (const group of groups) {
      for (let offset = 0; ; offset += 1000) {
        let query = supabaseAdmin.from("solve_history")
          .select("answer,reference_answer")
          .not("reference_answer", "is", null);
        if (group) query = query.in("student_id", group);
        if (since) query = query.gte("created_at", since);
        const { data, error } = await query.order("created_at", { ascending: false }).range(offset, offset + 999);
        if (error) throw error;
        const rows = (data || []) as SolveAnswer[];
        for (const row of rows) {
          if (!row.reference_answer?.trim()) continue;
          referenceCases++;
          if (answersMatch(row.answer || "", row.reference_answer)) referenceMatches++;
        }
        if (rows.length < 1000) break;
      }
    }
    return NextResponse.json({ referenceCases, referenceMatches });
  } catch (error) {
